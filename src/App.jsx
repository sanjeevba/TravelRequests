    ...
    <main className="request-card">
      ...
      <form onSubmit={submitRequest}>
        ...
        <div className="form-actions">
          ...
          <a className="button-link" href="/requests">My Requests</a>
        </div>
...
function ColumnResizer({ index, columns, setColumns, tableRef, label }) {
  ...
  function startResize(event) {
    ...
    const startWidth = (columns[index] / 100) * tableRef.current.getBoundingClientRect().width;
  ...
  function resizeWithKeyboard(event) {
    ...
    event.preventDefault();
    const width = (columns[index] / 100) * tableRef.current.getBoundingClientRect().width;
    resize(width + (event.key === 'ArrowLeft' ? -1 : 1) * (event.shiftKey ? 25 : 10));
  }
...
function RequestsTable() {
  ...
  useEffect(() => {
    ...
    async function loadRequests() {
      try {
        const response = await fetch('/api/travel-requests', { signal: controller.signal });
        const result = await readJsonResponse(response, 'Could not load requests.');
        setRequests(result);
        setMessage(result.length ? '' : 'No travel requests have been submitted.');
      } catch (error) {
        if (error.name !== 'AbortError') setMessage(error.message || 'Could not load requests.');
      }
    }
    loadRequests();
  ...
  const statusMessage = search && requests.length && !visibleRequests.length
  ...
  return (
    <main className="request-card">
      <div className="page-heading">
        <h2>My Requests</h2>
        <a className="button-link" href="/">New Request</a>
      </div>
      <div className="table-search">
        <label className="visually-hidden" htmlFor="requests-search">Search requests</label>
        <SearchIcon />
        <input
          id="requests-search"
          type="search"
      ...
      <p className="table-message" role="status" aria-live="polite">{statusMessage}</p>
      {requests.length > 0 && (
            ...
            <thead>
              <tr>
                {columnDefinitions.map(([key, label], index) => {
                  ...
                  return (
                    <th key={key} scope="col" aria-sort={active ? `${sort.direction}ending` : 'none'}>
                      <button
...
export default function App() {
  const isRequestsPage = ['/requests', '/requests.html'].includes(window.location.pathname);
  return (
    <>
      <Header />
